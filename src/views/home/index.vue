<template>
  <div class="dial">
    ...
    <div class="rotate">
      <!-- 转盘图片 -->
      <img class="dish" src="../../assets/turntable.png" :style="{ transform: rotate_deg, transition: rotate_transition }" />
      <!-- 指针图片 -->
    ...
    <div class="rule">
      ...
      <div class="rule_item">
        <p>1、只限于2023年8月份报名就业全程班的学员哦 </p>
        <p>2、每个人只有一次抽奖机会哈 </p>
        <p> 3、须在2023年8月31号前缴费进班哈</p>
      </div>
    </div>
  </div>
</template>
...
<script setup>
  ...
  const rotating = () => {
    ...
    var randomDeg = 360 - winningIndex * 60; //当前下标对应的角度    45是总共8个扇形区域，每个区域约45度
  ...
  const set = () => {
    ...
    // 0.3-0.7 800
    // 0.7-1 500
    if (__rand__ < 0.15) winIndex = 2; // 1000元
    else if (__rand__ < 0.3) winIndex = 5; // 1000元
    else if (__rand__ < 0.45) winIndex = 1; // 800元
    else if (__rand__ < 0.7) winIndex = 4; // 800元
    else if (__rand__ < 0.8) winIndex = 0; // 500元
    else if (__rand__ < 1) winIndex = 3; // 500元
    console.log('中奖下标', winIndex);
    return winIndex;
  };
</script>
